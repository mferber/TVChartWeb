...
// Implements PATCH
export async function updateWatchStatus(dataFilePath: string, showTitle: string, seasonNum: number, episodeIndex: number): Promise<void> {
  ...
  const updatedLines = origLines.map(line => {
    ...
    if (line.startsWith(showTitle)) {
      const newMarker = `, S${seasonNum}E${episodeIndex + 1}`;  // TODO this format needs to change to handle unnumbered specials
