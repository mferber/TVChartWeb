...
// Implements PATCH
export async function updateWatchStatus(dataFilePath: string, showTitle: string, seasonNum: number, episodeNum: number): Promise<void> {
  ...
  const updatedLines = origLines.map(line => {
    ...
    if (line.startsWith(showTitle)) {
      const newMarker = `, S${seasonNum}E${episodeNum}`;
